...
bool ReadSTLFile::ReadASCII(const char *buffer)
{
    ...
    do {
        ...
        if (useless != "facet")
            break;
        ss >> useless >> x >> y >> z;
        Point3.emplace_back(Point3f(x, y, z));
        getline(ss, useless);
        getline(ss, useless);
        for (i = 0; i < 3; i++)
        {
...
bool ReadSTLFile::ReadBinary(const char *buffer)
{
    ...
    {
        Point3.emplace_back(Point3f(cpyfloat(p), cpyfloat(p), cpyfloat(p)));
        for (j = 0; j < 3; j++)//读取三顶点
        {
            Point3.emplace_back(Point3f(cpyfloat(p), cpyfloat(p), cpyfloat(p)));
...
unsigned int ReadSTLFile::NumTri() const
{
    return unTriangles;
}

float ReadSTLFile::Length() const
{
    return length;
}

float ReadSTLFile::Width() const
{
    return width;
}

float ReadSTLFile::Height() const
{
    return height;
}
...
void ReadSTLFile::PrintSize() const
{
